...
export default function BranchLogin(){
  ...
  const submit = async (e) => {
    ...
    try{ await ensureCsrf(); await api.post('/api/auth/login/', { ...form, role:'BRANCH' }); navigate('/dashboard/branch') }catch(err){ setStatus('error:'+(err?.response?.data?.detail||'Login failed')) }
  }
  return (
